  ...
  return (
    <div className="relative overflow-hidden bg-indigo-50 pb-24">
      <div className="relative min-h-[calc(100vh-60px)] overflow-hidden bg-indigo-50">
        ...
        {/* --- LAYER KONTEN UTAMA --- */}
        <div className="relative z-10 flex flex-col items-center gap-6 px-4 py-20">
      ...
      <div className="space-y-2 bg-indigo-50/50">
        <motion.div
          initial={{ opacity: 0, y: 20 }}
        ...
        {/* 4 langkah dengan animasi muncul satu per satu */}
        <div className="mx-auto grid max-w-7xl grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
          {[1, 2, 3, 4].map((step) => (
            <motion.div
              ...
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ delay: 0.5 + step * 0.2 }}
              className="flex flex-col items-center justify-center gap-4 px-4 py-10 md:flex-row"
            >
              <Card className="flex h-full w-fit flex-col items-center bg-white/80 text-center backdrop-blur-sm transition-shadow hover:shadow-lg">
                <CardHeader className="flex flex-col items-center gap-2 pb-4">
                  {/* Lingkaran Angka */}
                  <div className="flex h-10 w-10 items-center justify-center rounded-full bg-indigo-100 text-xl font-bold text-indigo-700">
                    {step}
                  </div>
                  <CardTitle className="mt-4 w-64 text-sm font-bold text-slate-900">
                    Isi profil kamu
                  </CardTitle>
                </CardHeader>
